...
class _SociosElegiveisPageState extends State<SociosElegiveisPage> {
  ...
  Future<void> _apurarElegiveis() async {
    ...
    for (final membro in todosMembros) {
      // Regra 1: Ser sócio efetivo
      if (membro.situacaoSEAE != idSituacaoEfetivo) continue;

      // Regra 2: Ser sócio a pelo menos 4 anos
      if (membro.dataProposta.isEmpty) continue;
      ...
      // Regra 3: Participar de atividades (ter departamento)
      if (membro.atividades.isEmpty) continue;

      // Regra 4: Contribuição ininterrupta nos últimos 48 meses
    ...
    if (mounted) {
      setState(() {
        _membrosElegiveis = elegiveis..sort((a,b)=> a.nome.compareTo(b.nome));
      });
    }
  }

  // LÓGICA ATUALIZADA: Verificação estrita dos meses
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: _membrosElegiveis.isNotEmpty && !_isGeneratingPdf ? _gerarPdf : null,
            ),
          ],
        ),
        body: _buildBody(),
  ...
  List<DataRow> _createRows() {
    return _membrosElegiveis.map((membro) {
